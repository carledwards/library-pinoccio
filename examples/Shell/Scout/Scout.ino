...
static void hqConnectTimerHandler(SYS_Timer_t *timer) {
  if (!wifi.client.connected()) {
    leadHQConnect();
  }
}

static SYS_Timer_t hqConnectTimer;

void setup(void) {
  hqConnectTimer.interval = 1000;
  hqConnectTimer.mode = SYS_TIMER_PERIODIC_MODE;
  hqConnectTimer.handler = hqConnectTimerHandler;
  ...
  if (isLeadScout) {
    Serial.print("Initializing Wi-Fi backpack...");
    wifi.setup();
...
//  meshJoinGroup();
  ...
  if (isLeadScout && !skipLeadScoutCode) {
...
void loop(void) {
  ...
  Scout.loop();
  wifi.loop();
  if (isLeadScout && stayConnectedToHQ && !skipLeadScoutCode) {
    leadHQ();
  }
}
...
void leadHQConnect(void) {
  char auth[256], token[33];

  if (!wifi.client.connected()) {
    Serial.print("HQ connecting...");
    if (wifi.client.connect(server, hqPort)) {
...
numvar wifiConfig(void) {
  if (!wifi.apConfig((const char *)getstringarg(1), (const char *)getstringarg(2))) {
...
numvar wifiConnect(void) {
  if (!wifi.connectToAP()) {
     Serial.println("Error: connecting to AP failed");
  }
}
 ...
 *        HQ HANDLERS       *
\****************************/
numvar hqConnect(void) {
  SYS_TimerStart(&hqConnectTimer);
}

numvar hqDisconnect(void) {
  SYS_TimerStop(&hqConnectTimer);
}
